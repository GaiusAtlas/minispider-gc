...
class HtmlParseDriver(object):
    ...
    def update_Url(self,):
        ...
        try:
            ...
            self.spider_log.error("bs4 parse fail:{}".format(e))

        url_list_o = []
        self.depth += 1
        for id, i in enumerate(bs_read.find_all('a', {"href": True})):
            url_toget = i.attrs['href']
            url_o = self._trans_rela_to_abs(url_toget)
            if url_o != const.EMPTY:
...
if __name__ == '__main__':
    ...
    urln6 = 'http://slide.ent.sina.com.cn/star/h/slide_4_86512_370691.html'

    conf = conf_load.GetConf('spider.conf', 'spider')
    conf.conf_init()
    url_o = URL_List.URL_O(urln6, 0)
