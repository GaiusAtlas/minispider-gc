...
def getpic(url_pic):
    ...
    f = requests.get(url_pic, headers=header_to_take)
    print(f.status_code)
...
def geturladd(url_to_take, depth):
    url_list_o=[]
    header_to_take = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.88 Safari/537.36'}
    f = requests.get(url_to_take.url, headers=header_to_take)
    print(f.status_code)
    bs_readf = BeautifulSoup(f.text, 'lxml')
...
def downloadpic(url):
    ...
    try:
        with open('./output/{}'.format(pic_name), 'wb') as f:  # 这里文件名字要改一下，还要放到按网址建立的文件夹
            f.write(requests.get(url, headers=header_to_take).content)
    except IOError as ioe:
        print ('IO wrong')
